 ...
 * 典型全会聚角 0.08 rad。最后透镜到工件取 75 mm 是教学几何，不是公开工作距离。
 *
 * 一种近轴教学解释（这些数值不足以唯一确定真实物镜）：
 ...
 *
 *     X_focus = Feff · u          （u 为主光线倾角 rad，近轴时约等于坡度）
 *     AOI     = atan(-h / Feff)   （h 为相对机器光轴的入瞳偏心）
 *
 * h/u 参考机器光轴；AOI 是相对传播参考轴 -Z 的有符号角分量。
 * 工件水平时参考轴与表面法线平行；任意倾斜工件需另算光线与表面法线夹角。
 * EP3932609B1 [0039] 只给出“移束→偏心 d→聚焦后倾角→工件 AOI”的因果关系，
 * 没有公式或数值可用来证明本映射严格等价。Fig.3 的 α/β 也不是本页的 X/Y 分量。
 *
...
export interface PupilState {
  ...
  hxMm: number;
  hyMm: number;
  /** 主光线倾角（rad）；tan(u)=-dx/dz，tan(v)=-dy/dz，按近轴模型使用。 */
  u: number;
  v: number;
  /** 光束半径 mm。 */
  radiusMm: number;
