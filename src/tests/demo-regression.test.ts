...
import { AppState, commonTrace } from '../app-state';
...
describe('加工演示回归', () => {
  it('播放更新快照与焦点，暂停保持不动', () => {
    ...
    expect(commonTrace(after.trace).focusPoint.distanceTo(commonTrace(before.trace).focusPoint)).toBeGreaterThan(0.1);
    s.setPlaying(false);
    const paused = commonTrace(s.snapshot().trace).focusPoint.clone();
    s.advance(0.4);
    expect(commonTrace(s.snapshot().trace).focusPoint.distanceTo(paused)).toBeLessThan(1e-4);
  });
  ...
  for (const preset of PROCESS_PRESETS) {
    it(`${preset.name}：全周期光路命中实体镜面且无非目标遮挡`, () => {
      ...
      for (let step = 0; step <= 73; step++) {
        ...
        expect(commonTrace(snap.trace).ok).toBe(true);
        expect(Math.abs(snap.residual.xMm)).toBeLessThan(0.02);
        expect(Math.abs(snap.residual.alphaDeg)).toBeLessThan(0.02);
        view.update(snap.actuators.actuators as never, snap.trace.trace as never);
        view.group.updateMatrixWorld(true);
        const common = commonTrace(snap.trace);
        expect(common.hits.length).toBe(10);
        for (const hit of common.hits) {
        ...
        for (let i = 0; i < common.segments.length; i++) {
          const seg = common.segments[i];
